...
class GoogleAuthRepoImpl implements GoogleAuthRepo{
  ...
  GoogleAuthRepoImpl({required this.googleSignIn, required this.firebaseFirestore, required this.firebaseAuth});
  ...
  Future<Either<Failure, UserCredential>> googleAuthentication() async{
    try {
      ...
      if(user != null){
        ...
        final userModel = UserModel(uID: user.uid, firstName: firstName, lastName: lastName, username: user.displayName!, phone: user.phoneNumber ?? "0934567890", email: user.email ?? "");
        final userDoc = firebaseFirestore.collection("users").doc(userModel.uID);
        final userSnapshot = await userDoc.get();

        !userSnapshot.exists ? await userDoc.set(userModel.toJson()) : await userDoc.update({"lastSignIn": FieldValue.serverTimestamp()});
      }
